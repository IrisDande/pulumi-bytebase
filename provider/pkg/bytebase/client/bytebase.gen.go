...
// CollectionModel The CollectionModel describes the configuration and status of a bytebase collection.
...
// CreateCollectionRequest The configuration needed to create a bytebase collection.
...
// CreateIndexRequest The configuration needed to create a bytebase index.
...
// IndexModel The IndexModel describes the configuration and deployment status of a bytebase index.
...
type PodSpec struct {
	...
	// MetadataConfig Configuration for the behavior of bytebase's internal metadata index. By default, all metadata is indexed; when `metadata_config` is present, only specified metadata fields are indexed. These configurations are only valid for use with pod indexes.
